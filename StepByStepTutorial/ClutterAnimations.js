...
function changeRectanglePosition()
{
	...
	/*
	 * WARNING:
	 * The next code is deprectaed since clutter 1.6, but
	 * we don't have even the easy way to animate actors in
	 * raw clutter in stable version of fedora 17, so we use deprecated code.
	 *  The easy way in new code is:
		actorRectangle.animate(Clutter.AnimationMode.EASE_OUT_ELASTIC, 500,
                       "scale-x", 0.5,
                       "scale-y", 0.5,
                       NULL);
	 * New code to animate actors: http://docs.clutter-project.org/docs/clutter/stable/clutter-Implicit-Animations.html
	 */

	/*
	 * To animate an actor we need three things
	 * timeline: think about it like play, pause and stop and
	 * bar time of your videos
	 * alpha: how is made the transition, because we can made a tansition
	 * bounding, quickly at first and later slowly, etc.
	 * There are all the transitions: 
	 * http://docs.clutter-project.org/docs/clutter/stable/clutter-Implicit-Animations.html#ClutterAnimationMode
	 * behaviour: what property of the actor we want to animate, scale, opacity, etc. each property
	 * has his own class, behaviourScale, behaviourOpacity, etc.
	 * all properties of behaviourScale:
	...
	let behaviourScale = new Clutter.BehaviourScale( {'alpha':alpha, 'x_scale_start':1.0, 'y_scale_start':1.0, 'x_scale_end':0.5, 'y_scale_end':0.5} );
